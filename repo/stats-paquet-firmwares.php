...
if ($membre->_connected && $membre->_level > 0) {
	...
	$site_nom = config('nom');
	if($verif) {
		$firmwares = $pdo->prepare('SELECT firmware, COUNT(udid) AS udid FROM users INNER JOIN download ON download.user = users.id WHERE download.package = :id GROUP BY firmware ORDER BY firmware ASC');
		$firmwares->execute(array(':id' => $_GET['id']));
		...
		foreach($firmwares as $firmware){
			if(!empty($firmware['firmware']))
				...
				$data .= "{ y: 'iOS unknow',  x: ".$firmware['udid']."},";
		}
	}
				...
				<div class="tab-content jumbotron">
					<div class="tab-pane active">
						<div class="btn-group btn-group-justified hidden-lg hidden-xl" role="group">
							...
							<div class="btn-group" role="group">
								<a href="stats-paquet-firmwares.php?id=<?php echo $_GET['id']; ?>" class="btn btn-default active">Firmwares</a>
							</div>
							<div class="btn-group" role="group">
