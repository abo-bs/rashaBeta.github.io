...
if ($membre->_connected && $membre->_level > 0) {
	...
	if(isset($_GET['peryear'])) {
		...
		if($annee < date('Y', time()))
			...
			$anneesuiv = $annee;
		$req = $pdo->prepare('SELECT date, COUNT(id) FROM membre WHERE date LIKE :date GROUP BY MONTH(date) ORDER BY date ASC');
		$req->execute(array(':date' => $annee.'%'));
		$res = $req->fetchAll(PDO::FETCH_ASSOC);
		$req->closeCursor();
		$data = '';
		foreach($res as $ligne) {
			$news = (!empty($ligne['COUNT(id)'])) ? $ligne['COUNT(id)'] : 0;
			$data .= '{ month: \''.$ligne['date'].'\', new: '.$news.' },';
		}
		...
		$req = $pdo->prepare('SELECT date, COUNT(id) FROM membre WHERE date LIKE :date GROUP BY DAY(date) ORDER BY date ASC');
		...
		if($jour > $jours)
			$jour = $jours;
		$req = $pdo->prepare('SELECT date, COUNT(id) FROM membre WHERE date LIKE :date GROUP BY HOUR(date) ORDER BY date ASC');
		...
		foreach($res as $ligne) {
			...
			if($i<10)
				$i = '0'.$i;
			$data .= '{hour:\''.date_format(date_create($ligne['date']), "d-m-Y H:i").'\',new:'.$news.'},';
		}
	}
...
<html lang="fr">
<head>
	<title><?php echo _('Statistics');if(isset($_GET['peryear']))echo ' '.strtolower(_('Per year'));elseif(isset($_GET['permonth'])) echo ' '.strtolower(_('Per month'));else echo ' '.strtolower(_('Per day')); ?> - <?php echo $site_nom; ?></title>
<?php require_once('includes/admin/header.php'); ?>
	<div class="container">
		<div class="panel-heading">
			<h2 class="text-primary"><?php if(isset($_GET['peryear']))echo _('Statistics').' '.strtolower(_('Per year')).' : '.$annee;elseif(isset($_GET['permonth'])) echo _('Statistics').' '.strtolower(_('Per month')).' : '.$mois_affichage.' '.$annee;else echo _('Statistics').' '.strtolower(_('Per day')).' '.$jour.' '.$mois_affichage.' '.$annee; ?></h2>
		</div>
				...
				<div class="tab-content jumbotron">
					<div class="tab-pane active">
						<div class="btn-group btn-group-justified hidden-lg hidden-xl" role="group">
							...
							<div class="btn-group" role="group">
								<a href="stats-admin-membres.php" class="btn btn-default active"><?php echo _('Members'); ?></a>
							</div>
							<div class="btn-group" role="group">
						...
						<div class="btn-group btn-group-justified" role="group">
							<div class="btn-group" role="group">
								<a href="stats-admin-membres.php" class="btn btn-default<?php if(!isset($_GET['permonth']) && !isset($_GET['peryear'])) echo ' active'; ?>"><?php echo _('Per day'); ?></a>
							</div>
							<div class="btn-group" role="group">
								<a href="stats-admin-membres.php?permonth" class="btn btn-default<?php if(isset($_GET['permonth']) && !isset($_GET['peryear'])) echo ' active'; ?>"><?php echo _('Per month'); ?></a>
							</div>
							<div class="btn-group" role="group">
								<a href="stats-admin-membres.php?peryear" class="btn btn-default<?php if(isset($_GET['peryear'])) echo ' active'; ?>"><?php echo _('Per year'); ?></a>
							</div>
						</div>
						...
						<ul class="pager">
						<?php if($annee > 2014)
							echo '<li class="previous"><a class="alignleft" href="?month='.$moisprec.'&year='.$anneeprec.'&peryear">'._('Previous year').'</a></li>';
						if($annee < date('Y', time()))
							echo '<li class="next"><a class="alignright" href="?month='.$moissuiv.'&year='.$anneesuiv.'&peryear">'._('Next year').'</a></li>'; ?>
						</ul>
	...
	<script>new Morris.Area({element: 'stats_day',lineColors:['#f00','#00f'],data:[<?php echo $data; ?>],<?php if(isset($_GET['peryear'])) echo "xLabels:'month',";elseif(isset($_GET['permonth'])) echo "xLabels:'day',"; ?>xkey:'<?php if(isset($_GET['peryear'])) echo 'month';elseif(isset($_GET['permonth'])) echo 'day';else echo 'hour'; ?>',ykeys:['new'],labels: ['<?php echo _('New users'); ?>'],behaveLikeLine:true});</script>
	<?php require_once('includes/admin/footer.php'); ?>
