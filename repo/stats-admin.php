...
if ($membre->_connected && $membre->_level > 0) {
	...
	if(isset($_GET['peryear'])) {
		...
		if($annee < date('Y', time()))
			...
			$anneesuiv = $annee;
		$req = $pdo->prepare('SELECT date, COUNT(udid) FROM users WHERE date LIKE :date GROUP BY MONTH(date) ORDER BY date ASC');
		$req->execute(array(':date' => $annee.'%'));
		$res = $req->fetchAll(PDO::FETCH_ASSOC);
		$req->closeCursor();
		$data = '';
		foreach($res as $ligne) {
			$news = (!empty($ligne['COUNT(udid)'])) ? $ligne['COUNT(udid)'] : 0;
		...
		$req = $pdo->prepare('SELECT date, COUNT(udid) FROM users WHERE date LIKE :date GROUP BY DAY(date) ORDER BY date ASC');
		...
		if($jour > $jours)
			$jour = $jours;
		$req = $pdo->prepare('SELECT date, COUNT(udid) FROM users WHERE date LIKE :date GROUP BY HOUR(date) ORDER BY date ASC');
					...
					<div class="tab-pane active">
						<div class="btn-group btn-group-justified hidden-lg hidden-xl" role="group">
							<div class="btn-group" role="group">
								<a href="stats-admin.php" class="btn btn-default active"><?php echo _('Users'); ?></a>
							</div>
							<div class="btn-group" role="group">
						...
						<div class="btn-group btn-group-justified" role="group">
							<div class="btn-group" role="group">
								<a href="stats-admin.php" class="btn btn-default<?php if(!isset($_GET['permonth']) && !isset($_GET['peryear'])) echo ' active'; ?>"><?php echo _('Per day'); ?></a>
							</div>
							<div class="btn-group" role="group">
								<a href="stats-admin.php?permonth" class="btn btn-default<?php if(isset($_GET['permonth']) && !isset($_GET['peryear'])) echo ' active'; ?>"><?php echo _('Per month'); ?></a>
							</div>
							<div class="btn-group" role="group">
								<a href="stats-admin.php?peryear" class="btn btn-default<?php if(isset($_GET['peryear'])) echo ' active'; ?>"><?php echo _('Per year'); ?></a>
							</div>
						</div>
